...
def draw_legal(board_Array, index, dp):
    ...
    """
    for i in board_Array[index].get_moves():
       # print(f"Drawing legal move at index jere: {i}")
        if isinstance(board_Array[i], Empty):
            dp.draw_circle(i)
        else:
            dp.draw_sqr(i)
    return board_Array[index].get_moves()  # Return the list of legal moves for further processing if needed